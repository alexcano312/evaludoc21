...
//                return back()->withErrors(["generales" => "Ocurrió un error al guardar su información, inténtelo de nuevo más tarde."])->withInput();
    ...
    public function evaluarEvaluado (Request $request){
        ...
        if($request->tipo_evaluacion == "Docente"){
            ...
            for($x = 1; $x <= $request->nPreguntas; $x++){
                ...
                $arreglo = array("737","744","750","757");
                if(!$verificacionRespuesta){
                    if(in_array($inscripcion->id,$arreglo)){
                        if($valor < 2)
                        {
                            $valor = 2;
                        }
                    }
                    // -- Guardar respuesta con los valores recibidos
        ...
        // -- Si la evaluación es una actividad
            ...
            for($x = 1; $x <= $request->nPreguntas; $x++){
                ...
                // -- Guardar respuesta con lso valores recibidos
                $verificacionRespuestaActividad = Respuesta_Inscripcion_Actividad::where("alumno_inscripcion_actividad_id",$inscripcion->id)->where("pregunta_id",$preguntaId)->where("alumno_inscripcion_actividad_id",$inscripcion->id)->first();
    ...
    public  function codigoEvaluacion ($slug, $correo){
        ...
        $trayectoria = Trayectoria::where("matricula",Session::get("alumno")->matricula)->first();

        return view("alumno.codigo",["evaluacion" => $evaluacion, "codigo" => $codigo, "correo" => $correo, "trayectoria" => $trayectoria]);
