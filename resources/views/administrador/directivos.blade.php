	...
	<div class="content-wrap">
		<div class="container clearfix">
			<div class="table-responsive">
				<a href="#" id="abrir-registro" class="button button-border button-rounded button-green color-naranja" data-toggle="modal" data-target=".bs-example-modal-lg"><i class="icon-line-circle-plus"></i>Agregar Directivo</a>
				<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
					<div class="modal-dialog modal-lg">
						<div class="modal-body">
							<div class="modal-content">
								<div class="modal-header">
									<h4 class="modal-title" id="myModalLabel">Agregar Nuevo Directivo</h4>
									<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
								</div>
								<div class="modal-body">
									<p>Agrega la siguiente información para agregar un directivo nuevo</a>.</p>
									<form style="max-width: 100rem;" action="{{route("generales.agregar.directivo")}}" method="post">
										{{csrf_field()}}
				...
				<table id="datatable1" class="table table-striped table-bordered" cellspacing="0" width="100%">
					...
					<tbody>
						@foreach($directivos as $directivo)
							<tr class="">
								<td>{{$loop->index + 1}}</td>
								<td>{{$directivo->persona->nombre}} {{$directivo->persona->apellidos}}</td>
								<td>{{$directivo->persona->matricula}}</td>
								<td>{{$directivo->persona->correo}}</td>
								<td>{{$directivo->area->nombre}}</td>
								<td>
